...
export default function Navbar() {
    ...
    const JoinNow = () => (
      <Button asChild className={`transform skew-x-[-8deg] px-5 py-2 font-extrabold tracking-wide md:px-6 md:py-2
        ${applyScrolled ? "bg-neutral-900 text-white hover:bg-neutral-800" : "md:bg-sky-400 md:text-neutral-900 bg-neutral-900 text-white"}`}>
        <a href="#join"><span className="block skew-x-[8deg]">JOIN NOW</span></a>
      </Button>
    );
    ...
    return (
      ...
      >
        ...
        <a href="#home" className="flex items-center gap-3 md:hidden">
          <Image src="/cfcommune.png" alt="CrossFit Commune Logo" width={48} height={48} />
        </a>
        
        {/* Desktop Logo */}
        <a href="#home" className="hidden md:flex items-center gap-3">
          {scrolled ? (
            <Image src="/cfcommune.png" alt="CrossFit Commune Logo" width={60} height={60} />
          ) : (
            <Image src="/cfcommunetransparent.png" alt="CrossFit Commune Logo" width={120} height={120} />
          )}
        </a>
          ...
          <button
            ...
            className="md:hidden inline-flex items-center justify-center rounded-md p-2 ml-auto"
            onClick={() => setOpen((v) => !v)}
          >
        ...
        <div
          ...
          className={`md:hidden fixed inset-0 z-50 transition ${open ? "visible bg-black/40" : "invisible bg-transparent"}`}
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            className={`ml-auto h-full w-[85%] max-w-sm bg-white text-neutral-900 shadow-2xl transition-transform ${open ? "translate-x-0" : "translate-x-full"}`}
              ...
              {navItems.map((item) => (
                ...
                >
                  <span className="inline-flex items-center gap-2">
                    {item.label}
                    {item.label.toLowerCase().includes("programs") && <ChevronDown className="h-6 w-6" />}
                  </span>
                </a>
              ))}
              <JoinNow />
